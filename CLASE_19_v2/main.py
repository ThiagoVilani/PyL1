...
pygame.time.set_timer(tick_2, 1000)
tiempo = 0
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == tick:
            enemy.update()
        if event.type == tick:
            if len(cargador.lista_proyectiles) < 10 and 3 < tiempo :
                proyectil = Proyectil((enemy.pos_x + 140), (enemy.pos_y + 100))
                cargador.lista_proyectiles.append(proyectil.calcular_trayectoria(player_1.rect.x, player_1.rect.y))
...
#    screen.blit(imagen_fondo,imagen_fondo.get_rect())
    ...
    if 3 < tiempo:
        cargador.update(screen, player_1.rect.x, player_1.rect.y)
    player_1.update()
